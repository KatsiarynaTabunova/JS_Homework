...
{//3
    function doCalculation({a, b}, z = 1) {
        [x, y] = [a, b];
        return (x ** y) * z;
    }
...
{//6
    function countVowelLetters(text) {
        ...
        let result = [...text.toLowerCase()].filter(element =>
            vowelLetters.includes(element)
        );
...
{//7
    function findUsers(arr) {
        let arrUsersLess40 = arr.filter(user =>
            user.age < 40
        )
        let findFedor = arr.find(user =>
            user.name.startsWith('Fedor')
        )
        return {'Пользователи младше 40': arrUsersLess40, 'Пользователь с именем Федор': findFedor};
    }
